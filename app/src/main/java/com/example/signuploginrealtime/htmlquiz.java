...
//                if (option1btn.isPressed() || option2btn.isSelected() || option3btn.isSelected() || option4btn.isSelected()) {
//                    processAnswer();
//                    // Check if all questions are attempted
//                    if (questionAttempted < quizModalArrayList.size()) {
//                        currentPos++;
//                        setDataToViews(currentPos);
//                        clearSelection(); // Clear selection for the next question
//                    } else {
//                        nextBtn.setEnabled(true);
//                        submitBtn.setEnabled(true);
//                    }
//                } else {
//                    Toast.makeText(phpquiz.this, "Please select an option", Toast.LENGTH_SHORT).show();
//                }
//            }
//        });
...
//                    submitBtn.setVisibility(View.VISIBLE);
                ...
                DatabaseReference htmlRef = userRef.child("HTML");
                ...
                if (score <= 6) {
                    ...
                    tryAgain.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent BG = new Intent(getApplicationContext(), htmlquiz.class); //If User get 20% let him or her play again
                            startActivity(BG);
                        }
                    });
                    ...
                    htmlRef.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            int currentHtmlScore = dataSnapshot.exists() ? dataSnapshot.child("htmlScore").getValue(Integer.class) : 0;
                            currentHtmlScore += score;
                            htmlRef.child("htmlScore").setValue(currentHtmlScore);
                            htmlRef.child("status").setValue("completed");
                            ...
                            int finalCurrentHtmlScore = currentHtmlScore;
                            certificate.setOnClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View view) {
                                    Intent intent = new Intent(getApplicationContext(), certificate.class);
                                    intent.putExtra("HTML", "html");
                                    intent.putExtra("htmlScore", finalCurrentHtmlScore);
                        ...
                        public void onCancelled(@NonNull DatabaseError databaseError) {
                            // Handle error
                            Toast.makeText(getApplicationContext(), "Failed to update HTML score", Toast.LENGTH_SHORT).show();
                        }
                    });
...
//                }
//                showScore();
//                Toast.makeText(getApplicationContext(), "Your score: " + score + "/" + quizModalArrayList.size(), Toast.LENGTH_SHORT).show();

//                Intent intent = new Intent(phpquiz.this, userdb.class);
//                startActivity(intent);
                Objects.requireNonNull(resultView.getWindow()).setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                resultView.show();
