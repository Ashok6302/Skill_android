...
//                    submitBtn.setVisibility(View.VISIBLE);
                ...
                DatabaseReference phpRef = userRef.child("PHP");
                ...
                if (score <= 6) {
                    ...
                    tryAgain.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent BG = new Intent(getApplicationContext(), phpquiz.class); //If User get 20% let him or her play again
                            startActivity(BG);
                        }
                    });
                    ...
                    phpRef.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            int currentPhpScore = dataSnapshot.exists() ? dataSnapshot.child("phpScore").getValue(Integer.class) : 0;
                            currentPhpScore += score;
                            phpRef.child("phpScore").setValue(currentPhpScore);
                            phpRef.child("status").setValue("completed");
                        ...
                        public void onCancelled(@NonNull DatabaseError databaseError) {
                            // Handle error
                            Toast.makeText(getApplicationContext(), "Failed to update PHP score", Toast.LENGTH_SHORT).show();
                        }
                    });
